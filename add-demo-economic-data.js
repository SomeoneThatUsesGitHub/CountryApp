...
async function addDemoEconomicData() {
  console.log('Adding demo economic data for United States...');
  
  // Check if data already exists for country ID 1 (USA)
  const existingData = await db.select().from(schema.economicData).where(eq(schema.economicData.countryId, 1));
  ...
  if (existingData.length > 0) {
    ...
    // Add USA data
    const result = await db.insert(schema.economicData).values(usaDemoData);
    console.log('Added USA economic data');
  }
  
  // Check if data exists for Germany (country ID 2)
  const existingGermanyData = await db.select().from(schema.economicData).where(eq(schema.economicData.countryId, 2));
  ...
  if (existingGermanyData.length > 0) {
    ...
    // Add Germany data
    const result2 = await db.insert(schema.economicData).values(germanyData);
    console.log('Added Germany economic data');
  }
  
  // Check if data exists for Switzerland (country ID 6)
  const existingSwissData = await db.select().from(schema.economicData).where(eq(schema.economicData.countryId, 6));
  ...
  if (existingSwissData.length > 0) {
    ...
    const result3 = await db.insert(schema.economicData).values(switzerlandData);
