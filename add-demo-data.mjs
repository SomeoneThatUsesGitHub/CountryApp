...
async function addDemoEconomicData() {
  ...
  if (existingData.length > 0) {
    ...
    // Add USA data
    await db.insert(schema.economicData).values(usaDemoData);
  ...
  if (existingGermanyData.length > 0) {
    ...
    // Add Germany data
    await db.insert(schema.economicData).values(germanyData);
  ...
  if (existingSwissData.length > 0) {
    ...
    await db.insert(schema.economicData).values(switzerlandData);
